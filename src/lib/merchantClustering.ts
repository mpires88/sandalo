...
export function suggestCat(
  key: string,
  idx: Record<string, Array<{ key: string; cat: string }>>,
  threshold = 0.4,
): string {
...
export function clusterGroups(
  ...
  threshold = 0.45,
): { clusters: MerchantGroup[]; keyToCluster: Record<string, string> } {
  ...
  for (const idxs of Object.values(buckets)) {
    for (let a = 0; a < idxs.length; a++)
      for (let b = a + 1; b < idxs.length; b++)
        if (wordSim(groups[idxs[a]].key, groups[idxs[b]].key) >= threshold) union(idxs[a], idxs[b])
  }
  ...
  const clusters: MerchantGroup[] = Object.values(clusterMap).map(memberIdxs => {
    const repIdx = memberIdxs.reduce(
      (best, i) => ((groups[i].txns as unknown[]).length > (groups[best].txns as unknown[]).length ? i : best),
      memberIdxs[0],
    )
    ...
    const variants = memberIdxs.length > 1 ? memberIdxs.filter(i => i !== repIdx).map(i => groups[i].displayDesc) : []
    memberIdxs.forEach(i => {
      keyToCluster[groups[i].key] = rep.key
    })
