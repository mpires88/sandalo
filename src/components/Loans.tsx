...
export default function Loans({ clientId }: { clientId: string }) {
  ...
  function openEdit(l: LoanWithBalance) {
    setForm({
      ...
      default_rate: l.default_rate != null
  ...
  function calcPayment() {
    ...
    const t = parseInt(form.term_months)
  ...
  async function save() {
    ...
    if (!p || p <= 0) { setFormErr('Advance / principal amount must be > 0'); return }
    ...
    if (showScheduleFields && (!t || t <= 0)) { setFormErr('Term (months) is required'); return }
    if (!isInformal && (!pmt || pmt < 0)) { setFormErr('Payment amount is required'); return }
    ...
    if (loanId) {
      ...
      if (existingLinked) {
        ...
        const { data: nameMatch } = await supabase
          .from('categories').select('id').eq('name', acctName).eq('client_id', clientId).maybeSingle()
        ...
        if (nameMatch) {
          ...
          const { data: liabCats } = await supabase
            .from('categories').select('sort_order').eq('client_id', clientId).eq('pl_section', section)
          const maxOrder = (liabCats ?? []).reduce((m: number, c: { sort_order: number }) => Math.max(m, c.sort_order || 0), 0)
  ...
  const lbl: React.CSSProperties = {
    fontSize: 11, fontWeight: 600, color: D.charcoal, marginBottom: 4, display: 'block',
  }
    ...
    <div style={{ padding: '28px 32px', maxWidth: 1100 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: D.muted, margin: '3px 0 0' }}>Loans, merchant cash advances, lines of credit, and other obligations</p>
        </div>
      ...
      {loans.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 24 }}>
          {[
            ...
            <div key={c.label} style={{ background: D.card, border: `1px solid ${D.border}`, borderRadius: 8, padding: '14px 18px' }}>
              <div style={{ fontSize: 10.5, color: D.muted, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{c.label}</div>
      ...
      {!loading && loans.length === 0 && (
        <div style={{ textAlign: 'center', padding: '60px 0', color: D.muted }}>
          ...
          <div style={{ fontSize: 12, marginTop: 4 }}>Add a loan, MCA, line of credit, or other obligation to start tracking payments.</div>
        </div>
      )}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5 }}>
            <thead>
              <tr style={{ background: D.page }}>
                {['Name', 'Lender', 'Instrument', 'Original', 'Outstanding', 'Rate / Factor', 'Payment', 'Term', ''].map(h => (
                  <th key={h} style={{
                    padding: '9px 12px',
                    textAlign: ['Original', 'Outstanding', 'Rate / Factor', 'Payment', ''].includes(h) ? 'right' : 'left',
              ...
              {loans.map((l, i) => {
                ...
                return (
                  <tr key={l.id} style={{ borderBottom: i < loans.length - 1 ? `1px solid ${D.border}` : 'none' }}>
                    ...
                    <td style={{ padding: '10px 12px', textAlign: 'right', color: D.charcoal }}>{fmt(l.original_principal)}</td>
                    <td style={{ padding: '10px 12px', textAlign: 'right' }}>
                      <div style={{ color: isPaidOff ? D.green : D.red, fontWeight: 600 }}>{fmt(l.outstanding)}</div>
                      <div style={{ height: 3, background: D.border, borderRadius: 2, marginTop: 3, width: 60, marginLeft: 'auto' }}>
                    ...
                    <td style={{ padding: '10px 12px', textAlign: 'right', color: D.charcoal, fontSize: 11.5 }}>
                      {rateDisplay}
                      {isMca && l.holdback_pct ? <div style={{ color: D.muted, fontSize: 10 }}>{(l.holdback_pct * 100).toFixed(0)}% holdback</div> : null}
                    </td>
                    <td style={{ padding: '10px 12px', textAlign: 'right', color: isInformalDebt(l.instrument_type ?? 'term_loan') ? D.muted : D.charcoal }}>
                      {isInformalDebt(l.instrument_type ?? 'term_loan')
                        ? <span style={{ fontStyle: 'italic' }}>Irregular</span>
                        : <>{fmt(l.payment_amount)}<span style={{ color: D.muted, fontSize: 10.5 }}> /{FREQ_LABELS[l.payment_frequency].toLowerCase().slice(0, 2)}</span></>
                      }
                    </td>
                    ...
                    <td style={{ padding: '10px 12px', textAlign: 'right' }}>
                      <button onClick={() => openEdit(l)} style={{ background: 'none', border: 'none', color: D.steel, fontSize: 11, cursor: 'pointer', marginRight: 8, fontWeight: 500 }}>Edit</button>
                      <button onClick={() => deleteLoan(l.id)} style={{ background: 'none', border: 'none', color: D.red, fontSize: 11, cursor: 'pointer', fontWeight: 500 }}>Delete</button>
                    </td>
                  </tr>
                )
              })}
      ...
      {modal && (
        <div style={{
          ...
          <div style={{
            background: '#fff', borderRadius: 10, padding: '28px 28px 20px',
            width: 580, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.2)',
          }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px 16px' }}>
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6 }}>
                  {(Object.entries(INSTRUMENT_LABELS) as [InstrumentType, string][]).map(([k, label]) => (
                    <button key={k} onClick={() => setForm(f => ({ ...f, instrument_type: k }))} style={{
                      padding: '6px 4px', borderRadius: 5, fontSize: 11, fontWeight: 500, cursor: 'pointer',
                ...
                {itype === 'mca' && (
                  <div style={{ marginTop: 8, fontSize: 11, color: D.muted, background: D.page, padding: '6px 10px', borderRadius: 5 }}>
                    MCA uses a <strong>factor rate</strong> (e.g. 1.35 = repay 135% of advance). Repayment schedule is estimated from payment amount.
                  </div>
                )}
                {itype === 'loc' && (
                  <div style={{ marginTop: 8, fontSize: 11, color: D.muted, background: D.page, padding: '6px 10px', borderRadius: 5 }}>
                    Line of credit is revolving. Record the current outstanding draw as principal. No fixed amortization schedule.
                  </div>
                )}
                {itype === 'friends_family' && (
                  <div style={{ marginTop: 8, fontSize: 11, color: D.muted, background: D.page, padding: '6px 10px', borderRadius: 5 }}>
                    Informal personal loan — repayments are typically irregular with no fixed cadence. Payments are tracked via linked bank transactions on the liability account. Interest rate and term are optional.
                  </div>
                )}
                {itype === 'flat_fee' && (
                  <div style={{ marginTop: 8, fontSize: 11, color: D.muted, background: D.page, padding: '6px 10px', borderRadius: 5 }}>
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <input style={inp} placeholder={itype === 'mca' ? 'e.g. Kabbage MCA — Jan 2026' : 'e.g. SBA Loan, Equipment Financing'}
                  value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
              </div>

              <div style={{ gridColumn: '1 / -1' }}>
                <label style={lbl}>Lender / Provider</label>
                <input style={inp} placeholder="Bank, funder, or creditor name"
              ...
              {showFactorRate && (
                <>
                  <div>
                    ...
                    <input style={inp} type="number" min="1.001" step="0.001" placeholder="e.g. 1.35"
                      value={form.factor_rate}
                      onChange={e => setForm(f => ({ ...f, factor_rate: e.target.value }))} />
                  </div>
                  <div>
                    <label style={lbl}>Holdback % <span style={{ fontWeight: 400, color: D.muted }}>(of daily sales)</span></label>
                    <input style={inp} type="number" min="0" max="100" step="0.1" placeholder="e.g. 12"
              ...
              {showLoanType && (
                <div>
                  ...
                  <select style={inp} value={form.loan_type}
              ...
              {showScheduleFields && (
                <div>
                  ...
                  <input style={inp} type="number" min="1" step="1" placeholder="e.g. 12"
              ...
              {!isInformal && (
                <div>
                  ...
                  <select style={inp} value={form.payment_frequency}
              ...
              {showInterestRate && showLoanType && form.loan_type === 'amortizing' && (
                <div style={{ display: 'flex', alignItems: 'flex-end' }}>
                  <button onClick={calcPayment} style={{
                    background: D.page, border: `1px solid ${D.border}`, color: D.sage,
                    borderRadius: 5, padding: '7px 12px', fontSize: 11.5, cursor: 'pointer', fontWeight: 600,
                  }}>
                    ↻ Calculate Payment
                  </button>
                </div>
              ...
              <div>
                ...
                <input style={inp} type="number" min="0" step="0.01" placeholder="e.g. 2.5"
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 20 }}>
              <button onClick={() => setModal(false)} style={{
                ...
                borderRadius: 5, padding: '8px 16px', fontSize: 12.5, cursor: 'pointer',
              }}>Cancel</button>
              <button onClick={save} disabled={saving} style={{
                background: D.sage, color: '#fff', border: 'none',
                borderRadius: 5, padding: '8px 20px', fontSize: 12.5, fontWeight: 600,
