...
const D = {
  sage: '#2C5F52', gold: '#C8A96E', page: '#F5F0E8', border: '#E5DDD0',
  card: '#FFFFFF', muted: '#9B8FA0', charcoal: '#4A4A3F', green: '#16a34a',
  red: '#dc2626', steel: '#6B7280',
}
...
const EMPTY_FORM = {
  name: '', institution: '', last_four: '',
  account_type: 'checking' as AccountType,
  parent_category: '',
}

const lbl: React.CSSProperties = { fontSize: 11, fontWeight: 600, color: D.charcoal, textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: 4, display: 'block' }
const inp: React.CSSProperties = { width: '100%', padding: '7px 10px', border: `1px solid ${D.border}`, borderRadius: 5, fontSize: 13, color: D.charcoal, background: '#fff', boxSizing: 'border-box' }
...
export default function FinancialAccounts() {
  ...
  async function move(id: string, direction: 'up' | 'down') {
    ...
    const sectionAccts = accounts
      .filter(a => accountSection(a.account_type) === section)
      .slice() // already sorted by sort_order from DB
    ...
    await Promise.all([
      supabase.from('financial_accounts').update({ sort_order: b.sort_order }).eq('id', a.id).eq('client_id', CLIENT_ID),
      supabase.from('financial_accounts').update({ sort_order: a.sort_order }).eq('id', b.id).eq('client_id', CLIENT_ID),
    ])
    await load()
  }
    ...
    return (
      <section style={{ marginBottom: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <h2 style={{ margin: 0, fontSize: 13, fontWeight: 700, color: D.charcoal, textTransform: 'uppercase', letterSpacing: '0.6px' }}>
            {section === 'asset' ? 'Bank Accounts — Assets' : 'Credit & Lines of Credit — Liabilities'}
          </h2>
          <button onClick={openAdd} style={{
            background: D.sage, color: '#fff', border: 'none', borderRadius: 5,
            padding: '5px 12px', fontSize: 12, fontWeight: 600, cursor: 'pointer',
          }}>+ Add</button>
        </div>

        {rows.length === 0 ? (
          <div style={{ background: D.card, border: `1px solid ${D.border}`, borderRadius: 8, padding: '20px 16px', color: D.muted, fontSize: 13, textAlign: 'center' }}>
              ...
              <thead>
                <tr style={{ background: D.page }}>
                  {['', 'Account', 'Institution', 'Last 4', 'Type', 'BS Position', 'Transactions', ''].map((h, i) => (
                    <th key={i} style={{ padding: '8px 14px', textAlign: h === 'Transactions' ? 'right' : 'left', fontSize: 10.5, fontWeight: 700, color: D.muted, textTransform: 'uppercase', letterSpacing: '0.5px', borderBottom: `1px solid ${D.border}` }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={a.id} style={{ borderBottom: `1px solid ${D.border}` }}>
                    <td style={{ padding: '6px 8px 6px 14px', whiteSpace: 'nowrap' }}>
                      <button
                        ...
                        disabled={idx === 0}
                        style={{ background: 'none', border: 'none', color: idx === 0 ? D.border : D.muted, cursor: idx === 0 ? 'default' : 'pointer', fontSize: 10, padding: '1px 3px', lineHeight: 1 }}
                      >▲</button>
                      <button
                        onClick={() => move(a.id, 'down')}
                        disabled={idx === rows.length - 1}
                        style={{ background: 'none', border: 'none', color: idx === rows.length - 1 ? D.border : D.muted, cursor: idx === rows.length - 1 ? 'default' : 'pointer', fontSize: 10, padding: '1px 3px', lineHeight: 1 }}
                      >▼</button>
                    </td>
                    ...
                    <td style={{ padding: '10px 14px', color: D.steel, fontFamily: 'monospace' }}>{a.last_four ? `••••${a.last_four}` : '—'}</td>
                    <td style={{ padding: '10px 14px' }}>
                      <span style={{ background: `${color}15`, color, fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 10 }}>
                    ...
                    <td style={{ padding: '10px 14px', textAlign: 'right', whiteSpace: 'nowrap' }}>
                      <button onClick={() => openEdit(a)} style={{ background: 'none', border: `1px solid ${D.border}`, color: D.steel, borderRadius: 4, padding: '3px 8px', fontSize: 11, cursor: 'pointer', marginRight: 6 }}>Edit</button>
                      <button onClick={() => del(a.id)} style={{ background: 'none', border: `1px solid ${D.border}`, color: D.red, borderRadius: 4, padding: '3px 8px', fontSize: 11, cursor: 'pointer' }}>Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
    ...
    <div style={{ padding: '28px 32px', maxWidth: 960, margin: '0 auto' }}>
      ...
      {modal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}
          ...
          <div style={{ background: '#fff', borderRadius: 10, padding: '26px 26px 18px', width: 440, boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <h2 style={{ margin: '0 0 18px', fontSize: 15, fontWeight: 700, color: D.sage }}>{editing ? 'Edit Account' : 'Add Account'}</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                <label style={lbl}>Account Name *</label>
                <input style={inp} value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Chase Business Checking" />
              </div>
              <div>
                <label style={lbl}>Type *</label>
                <select style={inp} value={form.account_type} onChange={e => setForm(f => ({ ...f, account_type: e.target.value as AccountType, parent_category: '' }))}>
              ...
              <div>
                ...
                <select style={inp} value={form.parent_category} onChange={e => setForm(f => ({ ...f, parent_category: e.target.value }))}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  ...
                  <input style={inp} value={form.institution} onChange={e => setForm(f => ({ ...f, institution: e.target.value }))} placeholder="e.g. Chase" />
                </div>
                <div>
                  <label style={lbl}>Last 4 digits</label>
                  <input style={inp} value={form.last_four} onChange={e => setForm(f => ({ ...f, last_four: e.target.value.replace(/\D/g, '').slice(0, 4) }))} placeholder="1234" maxLength={4} />
                </div>
              </div>
            </div>
            {err && <div style={{ marginTop: 10, color: D.red, fontSize: 12 }}>{err}</div>}
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 20, paddingTop: 14, borderTop: `1px solid ${D.border}` }}>
              <button onClick={() => setModal(false)} style={{ background: 'none', border: `1px solid ${D.border}`, color: D.steel, borderRadius: 5, padding: '6px 14px', fontSize: 13, cursor: 'pointer' }}>Cancel</button>
              <button onClick={save} disabled={saving} style={{ background: D.sage, color: '#fff', border: 'none', borderRadius: 5, padding: '6px 16px', fontSize: 13, fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.7 : 1 }}>
                {saving ? 'Saving…' : 'Save'}
              </button>
            </div>
          </div>
