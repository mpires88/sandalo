...
import {
  PL_SECTIONS, BS_SECTIONS, DEFAULT_ACCOUNTS,
...
function buildDisplayList(sectionAccounts: MergedAccount[]): DisplayRow[] {
  ...
  sectionAccounts.filter(a => !placed.has(a.name)).sort(alpha).forEach(a => result.push({ account: a, depth: 0 }))
  return result
}
...
export default function ChartOfAccounts({ clientId }: { clientId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (missing.length && clientId) {
        const rows = missing.map(a => ({ client_id: clientId, name: a.name, sort_order: a.sort_order, pl_section: a.pl_section, parent: a.parent ?? null }))
  ...
  const seedDefaults = async () => {
    setSaving(true)
    const rows = DEFAULT_ACCOUNTS.map(a => ({ client_id: clientId, name: a.name, sort_order: a.sort_order, pl_section: a.pl_section, parent: a.parent ?? null }))
    const { error: err } = await supabase.from('categories').upsert(rows, { onConflict: 'name' })
    if (err) { alert('Seed failed: ' + err.message); setSaving(false); return }
  ...
  const addAccount = async (section: string) => {
    ...
    setSaving(true)
    const { error: err } = await supabase.from('categories').insert({ client_id: clientId, name, sort_order: maxOrder + 10, pl_section: section, parent: newParent || null })
    if (err) { alert('Could not add account: ' + err.message); setSaving(false); return }
  ...
  const saveEdit = async (oldName: string) => {
    ...
    if (err) { alert('Save failed: ' + err.message); setSaving(false); return }
    if (name !== oldName) {
  ...
  const changeSection = async (name: string, section: string) => {
    ...
    const detachFromParent = acc?.parent
      ? (() => { const p = accounts.find(a => a.name === acc.parent); return !p || p.pl_section !== section })()
      : false
    ...
    await supabase.from('categories').update({ pl_section: section, ...(detachFromParent ? { parent: null } : {}) }).eq('name', name)
    ...
    <div style={{ background: T.page, minHeight: '100%' }}>
      <header style={s.pageHeader}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          {accounts.length === 0 && (
            <button style={s.btnPrimary} onClick={seedDefaults} disabled={saving}>Seed Default Accounts</button>
          )}
        </div>
      </header>
      ...
      <div style={{ padding: '20px 28px', maxWidth: 860 }}>
        ...
        {accounts.length === 0 && !loading && (
          <div style={s.infoBox}>
            No accounts yet. Click <strong>Seed Default Accounts</strong> to pre-populate the standard chart of accounts for a spa business.
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 0, marginBottom: 20, borderBottom: `1px solid ${T.border}` }}>
          {[
            ...
            <button
              key={t.key}
              onClick={() => { setTab(t.key as 'pl'|'bs'); setAddingTo(null); setNewName(''); setNewParent('') }}
              style={{
          ...
          return (
            <div key={section} style={s.sectionCard}>
              <div style={s.sectionHead}>
                ...
                <button style={s.addBtn} onClick={() => { setAddingTo(addingTo === section ? null : section); setNewName(''); setNewParent('') }}>
                  + Add account
                </button>
              </div>
              ...
              {addingTo === section && (
                <div style={s.addForm}>
                  <input
                    autoFocus style={s.input} placeholder="Account name…"
                    value={newName} onChange={e => setNewName(e.target.value)}
                    onKeyDown={e => { if (e.key === 'Enter') addAccount(section); if (e.key === 'Escape') setAddingTo(null) }}
                  />
                  <select style={s.sectionSelect} value={newParent} onChange={e => setNewParent(e.target.value)} title="Optional: nest this under a parent account">
                  ...
                  <button style={s.btnPrimary} onClick={() => addAccount(section)} disabled={!newName.trim() || saving}>Add</button>
                  ...
                  {mergedList.map(row => {
                    ...
                    return (
                      ...
                      >
                          ...
                          {isEditing ? (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                              <input
                                ...
                                onKeyDown={e => { if (e.key === 'Enter') saveEdit(acc.name); if (e.key === 'Escape') setEditKey(null) }}
                              />
                              <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                                <span style={{ fontSize: 10, color: '#9ca3af', flexShrink: 0 }}>Sub-account of:</span>
                                <select style={{ ...s.sectionSelect, fontSize: 10, padding: '3px 7px' }} value={editParent} onChange={e => setEditParent(e.target.value)}>
                                  <option value="">— Top Level —</option>
                                ...
                                <button style={{ ...s.btnPrimary, padding: '3px 10px', fontSize: 10 }} onClick={() => saveEdit(acc.name)} disabled={saving}>Save</button>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', paddingLeft: depth * 20 }}>
                              ...
                              {isChild && (
                                <span style={{ marginLeft: 8, fontSize: 10, color: '#9ca3af', background: '#f1f5f9', borderRadius: 4, padding: '1px 7px', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                                  {acc.parent}
                                  <button title="Remove parent" onClick={() => changeParent(acc.name, null)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, color: '#9ca3af', padding: 0, lineHeight: 1 }}>×</button>
                                </span>
                              )}
                            </div>
                          )}
                        ...
                        <td style={{ ...s.td, whiteSpace: 'nowrap' }}>
                          <select style={s.sectionSelect} value={acc.pl_section} onChange={e => changeSection(acc.name, e.target.value)}>
                        ...
                        <td style={{ ...s.td, whiteSpace: 'nowrap' }}>
                          <button style={s.iconBtn} title="Edit" onClick={() => { setEditKey(acc.name); setEditName(acc.name); setEditParent(acc.parent ?? '') }}>
                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round">
                              <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
                              <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
                            </svg>
                          </button>
                          <button style={{ ...s.iconBtn, color: T.danger }} title="Delete" onClick={() => deleteAccount(acc.name)}>
                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round">
        ...
        <div style={{ ...s.sectionCard, marginTop: 8 }}>
          <div style={s.sectionHead}><span style={s.sectionLabel}>Add Account</span></div>
          <div style={s.addForm}>
            <input
              style={s.input} placeholder="Account name…"
            ...
            />
            <select style={s.sectionSelect} value={newSection} onChange={e => { setNewSection(e.target.value); setNewParent('') }}>
              <optgroup label="Income Statement">{PL_SECTIONS.map(sec => <option key={sec} value={sec}>{sec}</option>)}</optgroup>
              <optgroup label="Balance Sheet">{BS_SECTIONS.map(sec => <option key={sec} value={sec}>{sec}</option>)}</optgroup>
            </select>
            <select style={s.sectionSelect} value={newParent} onChange={e => setNewParent(e.target.value)} title="Optional: nest this under a parent account">
              <option value="">— Top Level —</option>
            ...
            <button style={s.btnPrimary} disabled={!(addingTo === '__global__' && newName.trim()) || saving} onClick={() => addAccount(newSection)}>Add Account</button>
          </div>
        </div>
      </div>
    </div>
...
const s = {
  pageHeader:    { display: 'flex' as const, justifyContent: 'space-between' as const, alignItems: 'flex-start' as const, padding: '14px 28px', background: '#FAFAF8', borderBottom: '1px solid #D9D4C8' },
  pageTitle:     { fontSize: 14, fontWeight: 600, color: '#2C5F52', margin: '0 0 2px' },
  pageSub:       { fontSize: 11, color: 'rgba(74,74,63,0.65)', margin: 0 },
  spinner:       { width: 28, height: 28, border: '2px solid #D9D4C8', borderTopColor: '#2C5F52', borderRadius: '50%', animation: 'spin .7s linear infinite' },
  ...
  infoBox:       { background: '#E8F0EE', border: '1px solid #B8D4CC', borderRadius: 6, padding: '10px 14px', fontSize: 11, color: '#2C5F52', marginBottom: 16 },
  sectionCard:   { background: '#FAFAF8', border: '1px solid #D9D4C8', borderRadius: 7, marginBottom: 12, overflow: 'hidden' as const },
  sectionHead:   { display: 'flex' as const, justifyContent: 'space-between' as const, alignItems: 'center' as const, padding: '8px 14px', background: '#F5F0E8', borderBottom: '1px solid #D9D4C8' },
  sectionLabel:  { fontSize: 9.5, fontWeight: 700, color: '#C8A96E', textTransform: 'uppercase' as const, letterSpacing: '.07em' },
  addBtn:        { fontSize: 11, fontWeight: 500, color: '#2C5F52', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 0' },
  addForm:       { display: 'flex' as const, gap: 8, padding: '10px 14px', alignItems: 'center' as const, borderBottom: '1px solid #F0EEE9', flexWrap: 'wrap' as const },
  input:         { flex: 1, padding: '5px 9px', border: '1px solid #D9D4C8', borderRadius: 5, fontSize: 12, color: '#2C5F52', background: '#fff', outline: 'none' },
  sectionSelect: { padding: '5px 9px', border: '1px solid #D9D4C8', borderRadius: 5, fontSize: 11, color: '#4A4A3F', background: '#fff', outline: 'none' },
  ...
  btnPrimary:    { padding: '5px 14px', background: '#2C5F52', color: '#fff', border: 'none', borderRadius: 5, fontSize: 11, fontWeight: 500, cursor: 'pointer' },
  btnSecondary:  { padding: '5px 12px', background: '#fff', color: '#4A4A3F', border: '1px solid #D9D4C8', borderRadius: 5, fontSize: 11, fontWeight: 500, cursor: 'pointer' },
}
