...
const COLOR_PRESETS = [
  '#2C5F52', '#4A7B6A', '#C8A96E', '#1A6EAD',
  '#B87400', '#7B4A8A', '#B94040', '#4A4A3F',
]
...
export default function ServiceCategories() {
    ...
    try {
      const [catRes, svcRes] = await Promise.all([
        supabase.from('service_categories').select('*')
          .eq('client_id', CLIENT_ID).order('sort_order').order('name').limit(200),
        supabase.from('services').select('category_id')
          .eq('client_id', CLIENT_ID).eq('is_active', true).limit(500),
      ])
  ...
  async function save() {
    ...
    try {
      const payload = {
        client_id: CLIENT_ID, name: form.name.trim(), color: form.color,
    ...
    try {
      ...
      if (err) {
        setDeleteErr(err.code === '23503'
          ? 'This category is used by existing services. Deactivate it instead.'
          : err.message)
        return
      }
            ...
            <thead>
              <tr style={{ background: '#F0EBE0', borderBottom: `1px solid ${D.border}` }}>
                {['Category', 'Services', 'Order', 'Status', ''].map(h => (
                  <th key={h} style={{ padding: '9px 14px', textAlign: 'left', fontWeight: 600, color: D.charcoal, fontSize: 11 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {visible.map((c, i) => (
                <tr key={c.id} style={{ borderBottom: i < visible.length - 1 ? `1px solid ${D.border}` : 'none', background: i % 2 === 0 ? D.card : '#F7F4EE', opacity: c.is_active ? 1 : 0.6 }}>
                  ...
                  <td style={{ padding: '11px 14px' }}>
                    <span style={{ background: c.is_active ? '#2E7D5214' : '#88888814', color: c.is_active ? D.green : '#888888', border: `1px solid ${c.is_active ? '#2E7D5240' : '#88888840'}`, borderRadius: 4, padding: '2px 8px', fontSize: 11, fontWeight: 500 }}>
                      {c.is_active ? 'Active' : 'Inactive'}
                    </span>
                  </td>
                  <td style={{ padding: '11px 14px', textAlign: 'right', whiteSpace: 'nowrap' }}>
                    <button onClick={() => openEdit(c)} style={actionBtn}>Edit</button>
                    <button onClick={() => toggleActive(c)} style={{ ...actionBtn, marginLeft: 6 }}>{c.is_active ? 'Deactivate' : 'Activate'}</button>
                    <button onClick={() => { setConfirmDelete(c); setDeleteErr('') }} style={{ ...dangerBtn, marginLeft: 6 }}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}
          ...
          <div style={{ background: '#fff', borderRadius: 10, width: 400, maxWidth: '95vw', boxShadow: '0 8px 32px rgba(0,0,0,0.18)' }}>
            <div style={{ padding: '20px 24px 14px', borderBottom: `1px solid ${D.border}` }}>
              <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: D.charcoal }}>{form.id ? 'Edit Category' : 'New Category'}</h2>
            </div>
            <div style={{ padding: '18px 24px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '13px 16px' }}>
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  ...
                  <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Massage" style={inp} autoFocus />
                </div>
                <div style={{ gridColumn: '1 / -1' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>
                    {COLOR_PRESETS.map(color => (
                      <button key={color} type="button" onClick={() => setForm(f => ({ ...f, color }))}
                        style={{ width: 24, height: 24, borderRadius: '50%', background: color, border: form.color === color ? '2px solid #4A4A3F' : '2px solid transparent', cursor: 'pointer', outline: 'none', padding: 0 }} />
                    ))}
                  </div>
                  <input type="color" value={form.color} onChange={e => setForm(f => ({ ...f, color: e.target.value }))} style={{ width: 40, height: 32, border: `1px solid ${D.border}`, borderRadius: 4, cursor: 'pointer', padding: 2 }} />
                  <span style={{ marginLeft: 8, fontSize: 12, color: D.muted }}>{form.color}</span>
                </div>
                <div>
                  <label style={lbl}>Sort Order</label>
                  <input type="number" min="0" value={form.sort_order} onChange={e => setForm(f => ({ ...f, sort_order: e.target.value }))} placeholder="1" style={inp} />
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingTop: 20 }}>
                  <input type="checkbox" id="cat-active" checked={form.is_active} onChange={e => setForm(f => ({ ...f, is_active: e.target.checked }))} style={{ cursor: 'pointer' }} />
                  <label htmlFor="cat-active" style={{ fontSize: 12.5, color: D.charcoal, cursor: 'pointer' }}>Active</label>
                </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}
          ...
          <div style={{ background: '#fff', borderRadius: 10, width: 360, maxWidth: '95vw', boxShadow: '0 8px 32px rgba(0,0,0,0.18)', padding: '24px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
              ...
              <button onClick={deleteCategory} disabled={deleting} style={{ background: D.red, color: '#fff', border: 'none', borderRadius: 6, padding: '8px 18px', fontSize: 13, fontWeight: 600, cursor: deleting ? 'not-allowed' : 'pointer', opacity: deleting ? 0.7 : 1 }}>
