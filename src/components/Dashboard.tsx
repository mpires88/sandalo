...
const D = {
  ...
  steel: '#4A7B6A', success: '#059669', danger: '#DC2626', amber: '#D97706',
}
const PIE_COLORS = ['#2C5F52','#C8A96E','#4A7B6A','#059669','#D97706','#DC2626','#3D7D7A','#6B6560','#2d6a9f','#c49a28']
...
function KpiCard({ label, value, sub, color = D.sage, warn = false }: {
...
const ttStyle = { background: D.sage, border: 'none', borderRadius: 5, color: '#fff', fontSize: 11, padding: '8px 12px' }

function CustomTip({ active, payload, label }: { active?: boolean; payload?: Array<{ name: string; value: number }>; label?: string }) {
...
interface SquareReport {
  period: string; gross_sales: number; net_sales: number
  tax_collected: number; fees: number; net_total: number
...
export default function Dashboard({ clientId }: { clientId: string }) {
  ...
  const monthlyPL = useMemo(() => {
    ...
    return Object.keys(byMonth).sort().map(ym => {
      ...
      return {
        ...
        revenue: netRev, cogs: -cogsSum, grossProfit,
        grossMarginPct: netRev > 0 ? (grossProfit / netRev * 100) : null,
        totalOpex: -opexSum, netProfit,
      }
    })
  }, [txns, sectionMap])
  ...
  const expenseByCategory = useMemo(() => {
    ...
    const topParent = (name: string): string => {
      ...
      let cur = name
      while (parentMap[cur] && !seen.has(cur)) { seen.add(cur); cur = parentMap[cur] }
      return cur
    }
    txns.forEach(t => {
      if (!t.account) return
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 300, background: D.page }}>
      <div style={{ width: 28, height: 28, border: `2px solid ${D.border}`, borderTopColor: D.sage, borderRadius: '50%', animation: 'spin .7s linear infinite' }} />
    </div>
  )
    ...
    <div style={{ background: D.page, minHeight: '100%' }}>
      ...
      <div style={{ padding: '20px 28px', maxWidth: 1160 }}>
        ...
        {(() => {
          ...
          const condCard = (
            label: string, target: string, value: string | null,
            met: boolean | null, color: string, progress: number,
          ) => (
            <div style={{ flex: '1 1 220px', background: D.page, border: `1px solid ${D.border}`, borderRadius: 7, padding: '12px 14px' }}>
              <div style={{ fontSize: 9.5, fontWeight: 700, color: D.gold, textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: 6 }}>{label}</div>
              <div style={{ fontSize: 11, color: 'rgba(74,74,63,0.6)', marginBottom: 6 }}>Target: {target}</div>
              <div style={{ fontSize: 18, fontWeight: 600, color, marginBottom: 8 }}>
                {value ?? '—'}
              </div>
              <div style={{ height: 5, background: D.border, borderRadius: 3, marginBottom: 6, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${Math.min(100, Math.max(0, progress))}%`, background: color, borderRadius: 3, transition: 'width .4s' }} />
              </div>
              ...
              <div style={{ background: D.card, border: `1px solid ${D.border}`, borderRadius: 8, padding: '16px 20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <div style={{ fontSize: 10, color: 'rgba(74,74,63,0.55)', marginTop: 2 }}>Target: {LEAF_T2_DATE}</div>
                  </div>
                </div>
                ...
                <div style={{ borderTop: `1px solid ${D.border}`, paddingTop: 10, display: 'flex', flexDirection: 'column', gap: 5 }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: D.gold, textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: 2 }}>Qualitative Conditions</div>
                  {[
                    'No event of default has occurred and is continuing',
                    'All representations and warranties remain true and correct in all material respects',
                  ].map((c, i) => (
                    <div key={i} style={{ fontSize: 11, color: D.charcoal, display: 'flex', gap: 7, alignItems: 'flex-start' }}>
          ...
          <>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={monthlyComparison} barCategoryGap="25%" barGap={3}>
                ...
                {years.map((yr, i) => (
                  <Bar key={yr} dataKey={String(yr)} fill={[D.border, D.steel, D.sage][i] ?? D.gold} radius={[3,3,0,0]} />
                ))}
              </BarChart>
            </ResponsiveContainer>
          </>
              ...
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={chartData} barCategoryGap="25%" barGap={2}>
                  ...
                  <Tooltip
                    ...
                    contentStyle={{ background: D.sage, border: 'none', borderRadius: 5, color: '#fff', fontSize: 11, padding: '8px 12px' }}
                    labelStyle={{ color: 'rgba(255,255,255,0.7)', fontWeight: 600, marginBottom: 4, fontSize: 10 }}
                    itemStyle={{ color: '#fff' }}
                  />
                  <Legend wrapperStyle={{ fontSize: 11 }} />
                  {allCats.map((cat, i) => (
                    <Bar key={cat} dataKey={cat} fill={catColors[i % catColors.length]} radius={[3,3,0,0]} stackId="a" />
                  ))}
                </BarChart>
              </ResponsiveContainer>
            </>
                ...
                <tbody>
                  {expenseByCategory.map((e, i) => (
                    <tr key={i} style={{ borderBottom: `1px solid ${D.border}` }}>
                      ...
                      <td style={{ padding: '5px 8px', textAlign: 'right', fontVariantNumeric: 'tabular-nums', fontSize: 11, color: 'rgba(74,74,63,0.55)' }}>{fmt(e.value / curRows.length)}</td>
                    </tr>
                  ))}
                  <tr style={{ background: D.page, fontWeight: 600 }}>
                    <td style={{ padding: '6px 8px', fontSize: 11, color: D.sage }}>Total</td>
                    <td style={{ padding: '6px 8px', textAlign: 'right', fontSize: 11, color: D.sage }}>{fmt(totalOpexCur)}</td>
                    <td style={{ padding: '6px 8px', textAlign: 'right', fontSize: 11, color: 'rgba(74,74,63,0.55)' }}>{fmt(totalOpexCur / curRows.length)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
