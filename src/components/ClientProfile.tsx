...
export default function ClientProfile({ customerId }: { customerId: string }) {
    ...
    try {
      const [cRes, aRes, fRes] = await Promise.all([
        ...
        supabase
          .from('appointments')
          .select(
            'id, appointment_date, start_time, duration_minutes, status, price_charged, tip_amount, deposit_paid, payment_method, services(name, price), staff(name)',
          )
  ...
  function openEdit() {
    ...
    setEditForm({
      ...
      ...Object.fromEntries(
        fieldDefs.map(f => [`cf_${f.field_key}`, String(customer.custom_fields?.[f.field_key] ?? '')]),
      ),
    })
    setFormErr('')
    setEditing(true)
  ...
  async function saveEdit() {
    ...
    fieldDefs.forEach(f => {
      const v = String(editForm[`cf_${f.field_key}`] ?? '')
      if (v) cf[f.field_key] = v
    })
    const { error: err } = await supabase
      .from('customers')
      .update({
        first_name: String(editForm.first_name).trim(),
        middle_name: String(editForm.middle_name).trim() || null,
        last_name: String(editForm.last_name).trim(),
        birthday: String(editForm.birthday) || null,
        email: String(editForm.email).trim() || null,
        phone: String(editForm.phone).trim() || null,
        phone_whatsapp: Boolean(editForm.phone_whatsapp),
        phone_alt: String(editForm.phone_alt).trim() || null,
        preferred_contact: String(editForm.preferred_contact) || null,
        preferred_language: String(editForm.preferred_language),
        address_street: String(editForm.address_street).trim() || null,
        address_city: String(editForm.address_city).trim() || null,
        address_state: String(editForm.address_state).trim() || null,
        address_zip: String(editForm.address_zip).trim() || null,
        allergies: String(editForm.allergies).trim() || null,
        preferences: String(editForm.preferences).trim() || null,
  ...
  if (error || !customer)
    return (
      <div style={{ padding: '48px 32px', background: D.page, minHeight: '100vh' }}>
        <Link href="/clients" style={{ color: D.steel, fontSize: 12.5, textDecoration: 'none' }}>
          ← Back to Clients
        </Link>
  ...
  return (
    <div style={{ background: D.page, minHeight: '100vh' }}>
      ...
      <div style={{ background: D.card, borderBottom: `1px solid ${D.border}`, padding: '16px 28px' }}>
        ...
        {/* Tabs */}
        <div
          style={{ display: 'flex', gap: 0, marginTop: 16, borderBottom: `1px solid ${D.border}`, marginBottom: -1 }}
        >
          {(
            [
              ['profile', 'Profile'],
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            <div style={{ background: D.card, border: `1px solid ${D.border}`, borderRadius: 8, padding: '18px 20px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 20px' }}>
                ...
                <Field
                  ...
                  value={CONTACT_LABELS[customer.preferred_contact ?? ''] ?? customer.preferred_contact ?? '—'}
                />
            ...
            >
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 40px' }}>
                <div>
                  <div
                    style={{
                      fontSize: 11,
                      fontWeight: 600,
                      color: customer.allergies ? D.red : D.muted,
                      marginBottom: 3,
                    }}
                  >
                ...
                <div>
                  <div style={{ fontSize: 11, fontWeight: 600, color: D.muted, marginBottom: 3 }}>
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
              {[
                ...
                <div
                  key={s.label}
                  style={{ background: D.card, border: `1px solid ${D.border}`, borderRadius: 7, padding: '8px 16px' }}
                >
                  <div style={{ fontSize: 10, color: D.muted, marginBottom: 1 }}>{s.label}</div>
              ...
              {(['all', 'scheduled', 'completed', 'no_show', 'cancelled'] as const).map(s => (
                <button
                  ...
                  style={{
                    ...
                    padding: '4px 10px',
                    fontSize: 11.5,
                    fontWeight: apptFilter === s ? 600 : 400,
                    cursor: 'pointer',
                  }}
                >
            ...
            <div style={{ background: D.card, border: `1px solid ${D.border}`, borderRadius: 8, overflow: 'hidden' }}>
              {filteredAppts.length === 0 ? (
                <div style={{ padding: 40, textAlign: 'center', color: D.muted, fontSize: 13 }}>
                  No appointments{apptFilter !== 'all' ? ` with status "${STATUS_LABELS[apptFilter]}"` : ''} on record.
                </div>
              ) : (
                    ...
                    {filteredAppts.map((a, i) => {
                      ...
                      return (
                        <tr
                          key={a.id}
                          style={{
                            borderBottom: i < filteredAppts.length - 1 ? `1px solid ${D.border}` : 'none',
                            background: i % 2 === 0 ? D.card : '#F7F4EE',
                          }}
                        >
                          <td style={{ padding: '9px 14px', whiteSpace: 'nowrap', color: D.charcoal }}>
                            {fmtDate(a.appointment_date)}
                          </td>
                          <td style={{ padding: '9px 14px', whiteSpace: 'nowrap', color: D.muted }}>
                          ...
                          <td
                            style={{
                              padding: '9px 14px',
                              color: Number(a.deposit_paid) > 0 ? D.charcoal : D.muted,
                              whiteSpace: 'nowrap',
                            }}
                          >
                            {Number(a.deposit_paid) > 0 ? fmtMoney(a.deposit_paid) : '—'}
                          </td>
                          <td
                            style={{
                              padding: '9px 14px',
                              color: a.price_charged != null ? D.charcoal : D.muted,
                              whiteSpace: 'nowrap',
                            }}
                          >
                          ...
                          <td
                            style={{
                              padding: '9px 14px',
                              color: Number(a.tip_amount) > 0 ? D.charcoal : D.muted,
                              whiteSpace: 'nowrap',
                            }}
                          >
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5 }}>
                    ...
                    {paymentRows.map((a, i) => {
                      ...
                      return (
                        <tr
                          key={a.id}
                          style={{
                            borderBottom: i < paymentRows.length - 1 ? `1px solid ${D.border}` : 'none',
                            background: i % 2 === 0 ? D.card : '#F7F4EE',
                          }}
                        >
                          <td style={{ padding: '9px 14px', whiteSpace: 'nowrap', color: D.charcoal }}>
                          ...
                          <td style={{ padding: '9px 14px', fontWeight: 600, color: D.charcoal, whiteSpace: 'nowrap' }}>
                  ...
                  <tfoot>
                    <tr style={{ background: '#F0EBE0', borderTop: `1px solid ${D.border}` }}>
                      <td
                        colSpan={3}
                        style={{ padding: '9px 14px', fontWeight: 600, color: D.charcoal, fontSize: 11.5 }}
                      >
                        Totals
                      </td>
                      <td style={{ padding: '9px 14px', fontWeight: 600, color: D.charcoal, whiteSpace: 'nowrap' }}>
                      ...
                      <td style={{ padding: '9px 14px', fontWeight: 700, color: D.sage, whiteSpace: 'nowrap' }}>
                        {fmtMoney(totalCharged + totalTips)}
                      </td>
                      <td />
                    </tr>
                  </tfoot>
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 16px' }}>
      ...
      <div>
        ...
        <input style={inp} value={f('first_name')} onChange={e => set('first_name', e.target.value)} />
      </div>
      <div>
        <label style={lbl}>Middle Name</label>
        <input style={inp} value={f('middle_name')} onChange={e => set('middle_name', e.target.value)} />
      </div>
      <div style={{ gridColumn: '1 / -1' }}>
        <label style={lbl}>
          Last Name <span style={{ color: D.red }}>*</span>
        </label>
        <input style={inp} value={f('last_name')} onChange={e => set('last_name', e.target.value)} />
      </div>
      ...
      <div>
        ...
        >
          <input
            type="checkbox"
            checked={Boolean(form.phone_whatsapp)}
            onChange={e => set('phone_whatsapp', e.target.checked)}
          />{' '}
          On WhatsApp
        </label>
      </div>
      <div>
        ...
        <input style={inp} value={f('phone_alt')} onChange={e => set('phone_alt', e.target.value)} />
      </div>
      <div style={{ gridColumn: '1 / -1' }}>
        <label style={lbl}>Email</label>
        <input style={inp} type="email" value={f('email')} onChange={e => set('email', e.target.value)} />
      </div>
      <div>
      ...
      <div style={{ gridColumn: '1 / -1' }}>
        ...
        <input style={inp} value={f('address_street')} onChange={e => set('address_street', e.target.value)} />
      </div>
      <div>
        <label style={lbl}>City</label>
        <input style={inp} value={f('address_city')} onChange={e => set('address_city', e.target.value)} />
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        ...
        <div>
          ...
          <input style={inp} value={f('address_zip')} onChange={e => set('address_zip', e.target.value)} />
        </div>
      </div>
      ...
      <div>
        ...
        <input style={inp} type="date" value={f('birthday')} onChange={e => set('birthday', e.target.value)} />
      </div>
      <div style={{ gridColumn: '1 / -1' }}>
        <label style={{ ...lbl, color: D.red }}>Allergies / Sensitivities</label>
        <input
          style={{ ...inp, borderColor: f('allergies') ? D.red + '80' : D.border }}
          value={f('allergies')}
          onChange={e => set('allergies', e.target.value)}
          placeholder="e.g. lavender oil, nut oils"
        />
      </div>
      <div style={{ gridColumn: '1 / -1' }}>
        ...
        <div key={fd.field_key}>
          ...
          {fd.field_type === 'boolean' ? (
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12.5 }}>
              <input
                type="checkbox"
                checked={form[`cf_${fd.field_key}`] === 'true'}
                onChange={e => set(`cf_${fd.field_key}`, String(e.target.checked))}
              />{' '}
              Yes
            </label>
            ...
            <select
              style={inp}
              value={String(form[`cf_${fd.field_key}`] ?? '')}
              onChange={e => set(`cf_${fd.field_key}`, e.target.value)}
            >
      ...
      <div style={{ gridColumn: '1 / -1', display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
        ...
        />
        <label htmlFor="edit-active" style={{ fontSize: 12.5, color: D.charcoal }}>
          Active
        </label>
      </div>
    </div>
