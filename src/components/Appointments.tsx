...
export default function Appointments() {
  ...
  const loadRef = useCallback(async () => {
    const [cRes, sRes, stRes, rRes, pmRes, aRes, acRes] = await Promise.all([
      ...
      supabase
        .from('services')
        .select(
          'id, name, price, duration_minutes, buffer_before_minutes, buffer_after_minutes, category, category_id, resource_id, staff_count, customer_count',
        )
        .eq('client_id', CLIENT_ID)
        .eq('is_active', true)
        .order('sort_order'),
      supabase.from('staff').select('id, name, status').eq('client_id', CLIENT_ID).order('name'),
      supabase.from('resources').select('id, name, quantity').eq('client_id', CLIENT_ID).order('name'),
      supabase
  ...
  const addonsForService = useCallback(
    (serviceId: string): ServiceAddon[] => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const { data, error: err } = await supabase
        .from('appointments')
        .select(
          '*, customers(first_name, middle_name, last_name), services(name, price, duration_minutes), staff(name)',
        )
        .eq('client_id', CLIENT_ID)
  ...
  }, [view, loadDay, loadDayBlocks])
  // biome-ignore lint/correctness/useExhaustiveDependencies: month is the reset trigger — clear the selected day when the month changes
  useEffect(() => {
  ...
  const apptIsPast =
    !!form.id && editingAppt != null && isPastAppt(editingAppt.appointment_date, editingAppt.start_time.slice(0, 5))
  ...
  async function openEdit(a: Appt) {
    ...
    setForm({
      ...
      tip_amount: String(a.tip_amount ?? 0),
      deposit_paid: String(a.deposit_paid ?? 0),
      payment_method: a.payment_method ?? '',
      square_transaction_id: a.square_transaction_id ?? '',
      notes: a.notes ?? '',
      extra_customers: [],
      extra_staff: [],
  ...
  async function save(force = false) {
    if (!form.customer_id) {
      setFormErr(t('Select a customer.', 'Seleccione un cliente.'))
      return
    }
    if (!form.service_id) {
      setFormErr(t('Select a service.', 'Seleccione un servicio.'))
      return
    }
    if (!form.staff_id) {
      setFormErr(t('Select a provider.', 'Seleccione un proveedor.'))
      return
    }
    if (!form.appointment_date) {
      setFormErr(t('Date is required.', 'La fecha es requerida.'))
      return
    }
    if (!form.start_time) {
      setFormErr(t('Time is required.', 'La hora es requerida.'))
      return
    }
    setSaving(true)
    setFormErr('')
    setDblBookWarn(null)
    try {
      const durMin = parseInt(form.duration_minutes, 10) || 60
      if (durMin <= 0) {
        setFormErr(
          t('Duration must be a positive number of minutes.', 'La duración debe ser un número positivo de minutos.'),
        )
        return
      }
      const [sh, sm] = form.start_time.split(':').map(Number)
      const newStart = sh * 60 + sm,
        newEnd = newStart + durMin
      ...
      if (isActiveBooking && form.resource_id && form.appointment_date && form.start_time) {
        ...
        const overlapping = (existing ?? []).filter(a => {
          ...
          const aStart = ah * 60 + am,
            aEnd = aStart + a.duration_minutes
        ...
        const starts = overlapping.map(a => {
          const [ah, am] = a.start_time.split(':').map(Number)
          return ah * 60 + am
        })
      ...
      if (new Set(allStaffIds).size !== allStaffIds.length) {
        setFormErr(
          t('Each party member needs a different provider.', 'Cada miembro del grupo necesita un proveedor diferente.'),
        )
        setSaving(false)
        return
      }
      ...
      if (!force && isActiveBooking) {
        ...
        if (clash) {
          ...
          const cust = (
            Array.isArray(clash.customers) ? (clash.customers[0] ?? null) : clash.customers
          ) as Appt['customers']
      ...
      if (form.id) {
        ...
        if (records.some(r => !r.staff_id)) {
          setFormErr(t('Select a provider for each slot.', 'Seleccione un proveedor para cada espacio.'))
          return
        }
  ...
  async function saveBlock() {
    if (!blockForm.staff_id) {
      setBlockErr(t('Select a provider.', 'Seleccione un proveedor.'))
      return
    }
    if (!blockForm.start_time) {
      setBlockErr(t('Start time is required.', 'La hora de inicio es requerida.'))
      return
    }
    if (!blockForm.end_time) {
      setBlockErr(t('End time is required.', 'La hora de fin es requerida.'))
      return
    }
    if (blockForm.end_time <= blockForm.start_time) {
      setBlockErr(t('End time must be after start time.', 'La hora de fin debe ser después de la hora de inicio.'))
      return
    }
    setBlockSaving(true)
    setBlockErr('')
    try {
      const payload = {
        ...
        label: blockForm.label.trim() || 'Blocked',
  ...
  function DayView() {
    ...
    const nowTop =
      isDayToday && nowMinutes >= DAY_START * 60 && nowMinutes < DAY_END * 60
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div>
                ...
                <div style={{ fontSize: 11.5, color: '#1A6EAD', opacity: 0.8 }}>
                  {findTime.serviceName} · {findTime.duration_minutes} min &mdash;{' '}
                  {t('click any open slot to book', 'haz clic en un espacio disponible para reservar')}
                </div>
              </div>
            </div>
            <button
                  ...
                  >
                    ...
                    {dayAppts.filter(a => a.staff_id === s.id).length > 0 && (
                      <span style={{ marginLeft: 6, fontSize: 10.5, color: D.muted, fontWeight: 400 }}>
                        {dayAppts.filter(a => a.staff_id === s.id).length}{' '}
                        {t(
                          ...
                          return (
                            <div
                              ...
                              style={{
                                ...
                                boxSizing: 'border-box',
                                background:
                                  'repeating-linear-gradient(45deg, rgba(74,74,63,0.07) 0px, rgba(74,74,63,0.07) 4px, transparent 4px, transparent 10px)',
                          ...
                          >
                            ...
                            {((a.status === 'scheduled' && a.appointment_date === today) ||
                              ...
                              <button
                                onClick={e => {
                                  e.stopPropagation()
                                  a.status === 'checked_in' ? openBill(a) : checkIn(a)
                                }}
                                style={{
                                  position: 'absolute',
                                  top: 4,
                                  right: 4,
                                  zIndex: 3,
                                  background: a.status === 'checked_in' ? '#0E7C7B' : D.green,
                                  color: '#fff',
                                  border: 'none',
                                  borderRadius: 3,
                                  padding: '1px 6px',
                              ...
                              >
                                {a.status === 'checked_in' ? t('Check Out', 'Finalizar') : t('Check In', 'Registrar')}
                              </button>
                            )}
                            <div
                              style={{
    ...
    return (
      <div>
        <div style={{ background: D.card, border: `1px solid ${D.border}`, borderRadius: 8, overflow: 'hidden' }}>
          ...
          >
            {t('Sun,Mon,Tue,Wed,Thu,Fri,Sat', 'Dom,Lun,Mar,Mié,Jue,Vie,Sáb')
              .split(',')
              .map(d => (
                <div
                  key={d}
          ...
          {Array.from({ length: cells.length / 7 }, (_, week) => (
            <div
              ...
              style={{
                display: 'grid',
                gridTemplateColumns: 'repeat(7, 1fr)',
                borderBottom: week < cells.length / 7 - 1 ? `1px solid ${D.border}` : 'none',
              }}
            >
                ...
                return (
                  ...
                  >
                    ...
                    {dayApptsList.length > 3 && (
                      <div style={{ fontSize: 10, color: D.muted, paddingLeft: 2 }}>
                        +{dayApptsList.length - 3} {t('more', 'más')}
                      </div>
                    )}
                  </div>
                )
        ...
        {selectedDate && (
          ...
          >
            ...
            >
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <span style={{ fontSize: 12.5, color: D.muted }}>
                  ...
                  {t(
                    selectedAppts.length !== 1 ? 'appointments' : 'appointment',
                    selectedAppts.length !== 1 ? 'citas' : 'cita',
                  )}
                </span>
                <button
                  onClick={() => switchToDay(selectedDate)}
                  style={{
                    background: 'transparent',
                    border: `1px solid ${D.border}`,
                    borderRadius: 5,
            ...
            {selectedAppts.length === 0 ? (
              <div style={{ padding: '20px 16px', color: D.muted, fontSize: 13 }}>
                {t('No appointments. Click "+ Add" to book one.', 'Sin citas. Haz clic en "+ Agregar" para reservar.')}
              </div>
            ) : (
                  ...
                  {selectedAppts.map((a, i) => {
                    ...
                    return (
                      <tr
                        key={a.id}
                        style={{
                          borderBottom: i < selectedAppts.length - 1 ? `1px solid ${D.border}` : 'none',
                          background: i % 2 === 0 ? D.card : '#F7F4EE',
                        }}
                      >
                        <td style={{ padding: '9px 14px', color: D.muted, whiteSpace: 'nowrap', width: 80 }}>
                          {fmtTime(a.start_time)}
                        </td>
                        <td style={{ padding: '9px 14px', fontWeight: 500, color: D.charcoal, whiteSpace: 'nowrap' }}>
                          {custName(a.customers)}
                        </td>
                        <td
                          style={{
                        ...
                        <td style={{ padding: '9px 14px', color: D.muted, whiteSpace: 'nowrap' }}>
                          {a.staff?.name ?? '—'}
                        </td>
                        <td style={{ padding: '9px 14px' }}>
                          ...
                          >
                            {t(STATUS_LABELS[a.status], STATUS_LABELS_ES[a.status])}
                          </span>
                        </td>
                        <td style={{ padding: '9px 14px', whiteSpace: 'nowrap' }}>
                          ...
                          <button onClick={() => openEdit(a)} style={{ ...actionBtn, fontSize: 11 }}>
                            {t('Edit', 'Editar')}
                          </button>
                        </td>
                      </tr>
      ...
      <>
        <div style={{ display: 'flex', gap: 4, marginBottom: 12, flexWrap: 'wrap' }}>
          {(['all', 'scheduled', 'completed', 'no_show', 'cancelled'] as const).map(s => (
            <button
              ...
              style={{
                background: statusFilter === s ? D.charcoal : 'transparent',
                color: statusFilter === s ? '#fff' : D.muted,
                border: `1px solid ${statusFilter === s ? D.charcoal : D.border}`,
                borderRadius: 5,
                padding: '4px 10px',
                fontSize: 12,
            ...
            >
              {s === 'all'
                ? t('All', 'Todos')
                : lang === 'es'
                  ? (STATUS_LABELS_ES[s] ?? STATUS_LABELS[s])
                  : STATUS_LABELS[s]}
            </button>
          ))}
        </div>
        <div style={{ background: D.card, border: `1px solid ${D.border}`, borderRadius: 8, overflow: 'hidden' }}>
          {loading ? (
            ...
            <div style={{ padding: 40, textAlign: 'center', color: D.muted, fontSize: 13 }}>
              {t('No appointments for', 'No hay citas para')} {monthLabel}
              {statusFilter !== 'all'
                ? ` · ${lang === 'es' ? (STATUS_LABELS_ES[statusFilter] ?? STATUS_LABELS[statusFilter]) : STATUS_LABELS[statusFilter]}`
                : ''}
              .
            </div>
          ) : (
              ...
              <tbody>
                {filtered.map((a, i) => {
                  ...
                  return (
                    ...
                    >
                      <td style={{ padding: '9px 12px', color: D.charcoal, whiteSpace: 'nowrap' }}>
                        {fmtDate(a.appointment_date)}
                      </td>
                      <td style={{ padding: '9px 12px', color: D.muted, whiteSpace: 'nowrap' }}>
                        {fmtTime(a.start_time)}
                      </td>
                      <td style={{ padding: '9px 12px', fontWeight: 500, color: D.charcoal, whiteSpace: 'nowrap' }}>
                        {custName(a.customers)}
                      </td>
                      <td
                        style={{
                      ...
                      <td style={{ padding: '9px 12px' }}>
                        ...
                        >
                          {(lang === 'es'
                            ? (STATUS_LABELS_ES[a.status] ?? STATUS_LABELS[a.status])
                            : STATUS_LABELS[a.status]) ?? a.status}
                        </span>
                      </td>
                      <td style={{ padding: '9px 12px', color: a.price_charged != null ? D.charcoal : D.muted }}>
                        {a.payment_method === 'insurance' ? (
                          <span style={{ fontSize: 11, color: '#1A6EAD', fontWeight: 500 }}>
                            {t('Insurance', 'Seguro')}
                          </span>
                        ) : (
                          fmtMoney(a.price_charged)
                        )}
                      </td>
                      <td style={{ padding: '9px 12px', color: Number(a.tip_amount) > 0 ? D.charcoal : D.muted }}>
                        {Number(a.tip_amount) > 0 ? fmtMoney(a.tip_amount) : '—'}
                      </td>
                      <td style={{ padding: '9px 12px', color: D.muted, fontSize: 11.5 }}>
                        {a.payment_method
                          ? (paymentMethods.find(pm => pm.code === a.payment_method)?.name ??
                            PAYMENT_LABELS_FALLBACK[a.payment_method] ??
                            a.payment_method)
                          : '—'}
                      </td>
                      <td style={{ padding: '9px 12px', whiteSpace: 'nowrap' }}>
                        {a.status === 'scheduled' && (
                          <>
                            ...
                            <button
                              onClick={() => quickNoShow(a.id)}
                              style={{ ...dangerBtn, marginRight: 5, fontSize: 11 }}
                            >
                              {t('No Show', 'No Asistió')}
                            </button>
                          </>
                        )}
                        {a.status === 'checked_in' && (
    ...
    <div style={{ padding: '28px 32px', background: D.page, minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            {(['calendar', 'day', 'log'] as const).map(v => (
              <button
                ...
                style={{
                  ...
                  border: 'none',
                  borderRight: v !== 'log' ? `1px solid ${D.border}` : 'none',
                  padding: '6px 14px',
                  fontSize: 12.5,
              ...
              >
                {v === 'calendar' ? t('Calendar', 'Calendario') : v === 'day' ? t('Day', 'Día') : t('Log', 'Registro')}
              </button>
            ))}
          </div>
          {view === 'day' && (
            <>
              <div style={{ display: 'flex', border: `1px solid ${D.border}`, borderRadius: 6, overflow: 'hidden' }}>
                {(['comfortable', 'compact'] as const).map(d => (
                  <button
                    key={d}
                    onClick={() => setDayDensity(d)}
                    style={{
                      background: dayDensity === d ? D.charcoal : '#fff',
                      color: dayDensity === d ? '#fff' : D.muted,
                      border: 'none',
                      borderRight: d === 'comfortable' ? `1px solid ${D.border}` : 'none',
                      padding: '6px 12px',
                      fontSize: 12,
                      fontWeight: dayDensity === d ? 600 : 400,
                      cursor: 'pointer',
                    }}
                  >
                    {d === 'comfortable' ? t('Comfortable', 'Cómodo') : t('Compact', 'Compacto')}
                  </button>
                ))}
              </div>
              {canWrite && (
          ...
          {canWrite && (
            <button
              onClick={() => openAdd(view === 'day' ? dayDate : undefined)}
              style={{
                background: D.sage,
                color: '#fff',
                border: 'none',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
        {view === 'day' ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <button onClick={() => setDayDate(d => addDays(d, -1))} style={navBtn}>
              ◀
            </button>
            ...
            {dayDate !== today && (
              <button onClick={() => setDayDate(today)} style={{ ...navBtn, padding: '4px 10px', fontSize: 11.5 }}>
                {t('Today', 'Hoy')}
              </button>
            )}
          </div>
          ...
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <button onClick={() => setMonth(m => new Date(m.getFullYear(), m.getMonth() - 1, 1))} style={navBtn}>
                ◀
              </button>
              <span style={{ fontSize: 15, fontWeight: 600, color: D.charcoal, minWidth: 148, textAlign: 'center' }}>
                {monthLabel}
              </span>
              <button onClick={() => setMonth(m => new Date(m.getFullYear(), m.getMonth() + 1, 1))} style={navBtn}>
                ▶
              ...
              {!isCurrentMonth && (
                <button
                  ...
                  style={{ ...navBtn, padding: '4px 10px', fontSize: 11.5 }}
                >
                  {t('Today', 'Hoy')}
                </button>
              )}
      ...
      {view === 'day' ? (
        ...
        loading ? (
          <div style={{ padding: 40, textAlign: 'center', color: D.muted }}>{t('Loading…', 'Cargando…')}</div>
        ) : error ? (
          ...
          >
            <div style={{ padding: '20px 24px 14px', borderBottom: `1px solid ${D.border}` }}>
              <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: D.charcoal }}>
                {form.id ? t('Edit Appointment', 'Editar Cita') : t('New Appointment', 'Nueva Cita')}
              </h2>
            </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '13px 16px' }}>
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  ...
                  >
                    ...
                    {customers.map(c => {
                      const mid = c.middle_name ? ` ${c.middle_name[0]}.` : ''
                      return (
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  ...
                  {form.resource_id &&
                    (() => {
                      ...
                      return r ? (
                        <div style={{ marginTop: 5, fontSize: 11.5, color: D.muted }}>
                          {t('Uses', 'Usa')}: {r.name} (max {r.quantity} {t('concurrent', 'concurrentes')})
                        </div>
                      ) : null
                    })()}
                </div>
                ...
                {form.service_id &&
                  (() => {
                    ...
                    const selectedTotal = form.addonIds.reduce(
                      ...
                      <div style={{ gridColumn: '1 / -1' }}>
                            ...
                            return (
                              <button
                                ...
                                onClick={() =>
                                  setForm(f => ({
                                    ...f,
                                    addonIds: sel ? f.addonIds.filter(x => x !== ad.id) : [...f.addonIds, ad.id],
                                  }))
                                }
                                style={{
                                  padding: '5px 11px',
                        ...
                        {form.addonIds.length > 0 && (
                          <div style={{ fontSize: 11.5, color: D.muted, marginTop: 6 }}>
                            {form.addonIds.length} {t('add-on', 'complemento')}
                            {form.addonIds.length !== 1 ? 's' : ''} · +{fmtMoney(selectedTotal)}
                          </div>
                        )}
                      </div>
                    )
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  <label style={lbl}>
                    {form.extra_staff.length > 0 ? t('Provider 1 *', 'Proveedor 1 *') : t('Provider *', 'Proveedor *')}
                  </label>
                  <select
                    value={form.staff_id}
                ...
                {form.extra_staff.map((sid, i) => (
                  <div
                    key={i}
                    style={{ gridColumn: '1 / -1', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 16px' }}
                  >
                    <div
                      style={{
                        gridColumn: '1 / -1',
                        fontSize: 10,
                        fontWeight: 700,
                        color: 'rgba(74,74,63,0.45)',
                        textTransform: 'uppercase',
                        letterSpacing: '0.8px',
                        paddingBottom: 5,
                        borderBottom: '1px dashed #D9D4C8',
                        marginBottom: 8,
                      }}
                    >
                      {t('Party', 'Grupo')} {i + 2}
                    ...
                    <div>
                      <label style={lbl}>
                        {t('Provider', 'Proveedor')} {i + 2} *
                      </label>
                      <select
                        value={sid}
                        onChange={e =>
                          setForm(f => {
                            const s = [...f.extra_staff]
                            s[i] = e.target.value
                            return { ...f, extra_staff: s }
                          })
                        }
                        style={inp}
                      >
                    ...
                    {form.extra_customers.length > i && (
                      <div>
                        <label style={lbl}>
                          {t('Customer', 'Cliente')} {i + 2}
                        </label>
                        <select
                          value={form.extra_customers[i]}
                          onChange={e =>
                            setForm(f => {
                              const c = [...f.extra_customers]
                              c[i] = e.target.value
                              return { ...f, extra_customers: c }
                            })
                          }
                          style={inp}
                        >
                ...
                {form.group_id && (
                  <div
                    style={{
                      ...
                      color: '#7B4A8A',
                      background: '#7B4A8A0A',
                      border: '1px solid #7B4A8A30',
                      borderRadius: 5,
                      padding: '6px 10px',
                    }}
                  >
                    {t(
                      'Part of a group booking — editing this appointment only.',
                      'Parte de una reserva grupal — editando solo esta cita.',
                    )}
                  </div>
                )}
                ...
                <div style={{ ...secHead, marginTop: 2, display: 'flex', alignItems: 'baseline', gap: 8 }}>
                  ...
                  {apptIsPast && (
                    <span style={{ fontWeight: 400, textTransform: 'none', letterSpacing: 0, color: D.muted }}>
                      {t('· Past appointment — cannot reschedule', '· Cita pasada — no se puede reprogramar')}
                    </span>
                  )}
                </div>
                <div>
                  ...
                  <input
                    ...
                    style={{
                      ...inp,
                      ...(apptIsPast ? { background: '#F3F1EB', color: D.muted, cursor: 'not-allowed' } : {}),
                    }}
                  />
                </div>
                <div>
                ...
                <div>
                  <label style={lbl}>{t('Duration (min)', 'Duración (min)')}</label>
                  <input
                    type="number"
                    min={1}
                ...
                <div>
                  ...
                  <select
                    ...
                    style={inp}
                  >
                    <option value="scheduled">{t('Scheduled', 'Programado')}</option>
                    <option value="completed">{t('Completed', 'Completado')}</option>
                    <option value="no_show">{t('No Show', 'No Asistió')}</option>
                    <option value="cancelled">{t('Cancelled', 'Cancelado')}</option>
                  </select>
                </div>
                <div style={{ ...secHead, marginTop: 2 }}>{t('Payment', 'Pago')}</div>
                <div>
                  <label style={lbl}>{t('Price Charged', 'Precio Cobrado')}</label>
                  <input
                    type="number"
                    step="0.01"
                ...
                <div>
                  <label style={lbl}>{t('Tip', 'Propina')}</label>
                  <input
                    type="number"
                    step="0.01"
                    value={form.tip_amount}
                    onChange={e => setForm(f => ({ ...f, tip_amount: e.target.value }))}
                    placeholder="0.00"
                    style={inp}
                  />
                </div>
                <div>
                  <label style={lbl}>{t('Deposit Paid', 'Depósito Pagado')}</label>
                  <input
                    type="number"
                    step="0.01"
                ...
                <div>
                  <label style={lbl}>{t('Payment Method', 'Método de Pago')}</label>
                  <select
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  <label style={lbl}>{t('Square Transaction ID', 'ID de Transacción Square')}</label>
                  <input
                    value={form.square_transaction_id}
                    onChange={e => setForm(f => ({ ...f, square_transaction_id: e.target.value }))}
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  ...
                  <textarea
                    ...
                    placeholder={t(
                      'Session notes, client feedback, etc.',
                      'Notas de sesión, comentarios del cliente, etc.',
                    )}
                    rows={2}
      ...
      {blockModal && (
        <div
          ...
          onClick={e => {
            if (e.target === e.currentTarget) setBlockModal(false)
          }}
        >
          <div
            style={{
          ...
          >
            <div style={{ padding: '20px 24px 14px', borderBottom: `1px solid ${D.border}` }}>
              <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: D.charcoal }}>
                {blockForm.id ? t('Edit Block', 'Editar Bloqueo') : t('Block Time', 'Bloquear Horario')}
              </h2>
            </div>
                ...
                >
                  ...
                  {(() => {
                    ...
                    return (
                      <button
                        onClick={() =>
                          setBlockForm(f => ({ ...f, start_time: biz.open_time, end_time: biz.close_time }))
                        }
                        style={{
                          background: 'none',
                          border: `1px solid ${D.border}`,
                        ...
                        onMouseEnter={e => {
                          e.currentTarget.style.color = D.charcoal
                          e.currentTarget.style.borderColor = D.charcoal
                        }}
                        onMouseLeave={e => {
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, marginBottom: 7, flexWrap: 'wrap' }}>
                    {(
                      ...
                      <button
                        key={en}
                        onClick={() => setBlockForm(f => ({ ...f, label: t(en, es) }))}
                        style={{
                          background:
                            blockForm.label === t(en, es) || blockForm.label === en || blockForm.label === es
                              ? D.charcoal
                              : 'transparent',
                          color:
                            blockForm.label === t(en, es) || blockForm.label === en || blockForm.label === es
                              ? '#fff'
                              : D.muted,
                          border: `1px solid ${blockForm.label === t(en, es) || blockForm.label === en || blockForm.label === es ? D.charcoal : D.border}`,
                          borderRadius: 4,
                          padding: '3px 9px',
                          fontSize: 11.5,
                          cursor: 'pointer',
                  ...
                  <input
                    ...
                    placeholder={t('Custom label…', 'Etiqueta personalizada…')}
                    style={inp}
                  />
                </div>
                <div style={{ gridColumn: '1 / -1' }}>
                  ...
                  <textarea
                    ...
                    placeholder={t('Optional details…', 'Detalles opcionales…')}
                    rows={2}
                    style={{ ...inp, resize: 'vertical', fontFamily: 'inherit' }}
                  />
                </div>
      ...
      {confirmDelBlock && (
        <div
          ...
          onClick={e => {
            if (e.target === e.currentTarget) setConfirmDelBlock(null)
          }}
        >
          <div
            style={{
          ...
          >
            <h2 style={{ margin: '0 0 10px', fontSize: 16, fontWeight: 700, color: D.charcoal }}>
              {t('Remove Time Block?', '¿Eliminar Bloqueo?')}
            </h2>
            <p style={{ margin: '0 0 20px', fontSize: 13, color: D.charcoal }}>
              <strong>{confirmDelBlock.label}</strong> {t('on', 'el')} {fmtDate(confirmDelBlock.block_date)},{' '}
              {fmtTime(confirmDelBlock.start_time)} – {fmtTime(confirmDelBlock.end_time)}{' '}
              {t('will be removed.', 'será eliminado.')}
            </p>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
              <button
                onClick={() => setConfirmDelBlock(null)}
                style={{
                  background: 'transparent',
                  border: `1px solid ${D.border}`,
                  borderRadius: 6,
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: 13,
                  fontWeight: 600,
                  cursor: deletingBlock ? 'not-allowed' : 'pointer',
                  opacity: deletingBlock ? 0.7 : 1,
                }}
              >
      ...
      {visitAppt && (
        <div
          ...
          onClick={e => {
            if (e.target === e.currentTarget) setVisitAppt(null)
          }}
        >
          <div
            style={{
          ...
          >
            ...
            >
              ...
              <div style={{ minWidth: 0 }}>
                ...
                >
                  {custName(visitAppt.customers)}
                </h2>
                <div style={{ fontSize: 12, color: D.muted }}>
                  {visitAppt.staff?.name ?? ''} · {fmtTime(visitAppt.start_time)} · {visitAppt.duration_minutes} min
                </div>
              </div>
            </div>
            <div style={{ padding: '18px 24px' }}>
                ...
                <>
                  ...
                  {visitAddons.map(ad => (
                    ...
                    >
                      <span>
                        {ad.name}
                        {ad.duration_minutes > 0 ? ` · +${ad.duration_minutes} min` : ''}
                      </span>
        ...
        (() => {
          ...
          return (
            <div
              ...
              onClick={e => {
                if (e.target === e.currentTarget) setBillAppt(null)
              }}
            >
              <div
                style={{
              ...
              >
                <div style={{ padding: '20px 24px 14px', borderBottom: `1px solid ${D.border}` }}>
                  <h2 style={{ margin: '0 0 3px', fontSize: 16, fontWeight: 700, color: D.charcoal }}>
                    {t('Checkout', 'Pago')}
                  </h2>
                  <div style={{ fontSize: 12.5, color: D.muted }}>
                    {custName(billAppt.customers)} · {fmtDate(billAppt.appointment_date)} {t('at', 'a las')}{' '}
                    {fmtTime(billAppt.start_time)}
                  </div>
                </div>
                <div style={{ padding: '18px 24px' }}>
                  {/* Line items */}
                  <div
                    style={{ border: `1px solid ${D.border}`, borderRadius: 8, overflow: 'hidden', marginBottom: 14 }}
                  >
                    {billLines.map((l, i) => (
                      <div
                        key={i}
                        style={{
                          ...
                          color: D.charcoal,
                          borderBottom: `1px solid ${D.border}`,
                          background: i % 2 ? '#F7F4EE' : '#fff',
                        }}
                      >
                        <span>{l.label}</span>
                        <span style={{ fontVariantNumeric: 'tabular-nums' }}>{fmtMoney(l.amount)}</span>
                      </div>
                    ))}
                    <div
                      style={{
                    ...
                    >
                      ...
                      <span style={{ fontVariantNumeric: 'tabular-nums', fontWeight: 600 }}>{fmtMoney(subtotal)}</span>
                    </div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 16px' }}>
                    ...
                    <div>
                      <label style={lbl}>{t('Payment Method', 'Método de Pago')}</label>
                      <select value={billPayment} onChange={e => setBillPayment(e.target.value)} style={inp}>
                        {paymentMethods.map(pm => (
                          <option key={pm.code} value={pm.code}>
                            {pm.name}
                          </option>
                  ...
                  <div
                    style={{
                      ...
                      justifyContent: 'space-between',
                      alignItems: 'baseline',
                      marginTop: deposit > 0 ? 8 : 14,
                      paddingTop: 12,
                      borderTop: `2px solid ${D.border}`,
                    }}
                  >
                    <span style={{ fontSize: 14, fontWeight: 700, color: D.charcoal }}>
                      {deposit > 0 ? t('Total Due', 'Total a Pagar') : t('Total', 'Total')}
                    </span>
                    <span
                      style={{ fontSize: 20, fontWeight: 700, color: '#0E7C7B', fontVariantNumeric: 'tabular-nums' }}
                    >
                      {fmtMoney(total)}
                    </span>
                  </div>
                  ...
                  <button
                    disabled
                    title={t('Square integration coming soon', 'Integración con Square próximamente')}
                    style={{
                      width: '100%',
                      marginTop: 14,
                      background: '#F3F1EB',
                  ...
                  >
                    {t('Pay with Square', 'Pagar con Square')}{' '}
                    <span style={{ fontSize: 10.5, fontWeight: 500 }}>({t('coming soon', 'próximamente')})</span>
                  </button>
                </div>
                <div
                  style={{
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      ...
                      fontSize: 13,
                      fontWeight: 600,
                      cursor: billSaving ? 'not-allowed' : 'pointer',
                      opacity: billSaving ? 0.7 : 1,
                    }}
                  >
