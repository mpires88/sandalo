...
function parseSquareEml(text: string): ParsedReport {
  ...
  if (htmlIdx === -1) return { error: 'No HTML body found in file', period: null, grossSales: 0, returns: 0, discounts: 0, netSales: 0, taxCollected: 0, fees: 0, netTotal: 0, cashAmount: 0, cardAmount: 0, categories: [] }
...
interface DbReport {
  id: string; period: string; gross_sales: number; net_sales: number
...
export default function SquareReports({ clientId }: { clientId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    const { data } = await supabase
      .from('square_reports').select('*').eq('client_id', clientId).order('period', { ascending: false })
    setReports(data ?? [])
    setLoading(false)
  }, [clientId])
  ...
  return (
    <div style={{ background: T.page, minHeight: '100%' }}>
      ...
      <div style={{ padding: '20px 28px', maxWidth: 860 }}>
        ...
        {!preview && (
          <div
            style={{ border: `2px dashed ${dragOver ? T.sage : T.border}`, borderRadius: 8, padding: '40px 24px', textAlign: 'center', background: dragOver ? '#E8F0EE' : T.card, cursor: 'pointer', marginBottom: 24, transition: 'border-color .2s, background .2s' }}
        ...
        {preview && (
          <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 8, padding: '18px 20px', marginBottom: 24 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
              <h3 style={{ fontSize: 13, fontWeight: 600, color: T.sage, margin: 0 }}>Preview — {fmtPeriod(preview.period!)}</h3>
              <div style={{ display: 'flex', gap: 8 }}>
                <button style={{ padding: '5px 14px', background: '#fff', color: T.charcoal, border: `1px solid ${T.border}`, borderRadius: 5, fontSize: 11, cursor: 'pointer' }} onClick={() => setPreview(null)}>Cancel</button>
                <button style={{ padding: '5px 16px', background: T.sage, color: '#fff', border: 'none', borderRadius: 5, fontSize: 11, fontWeight: 500, cursor: 'pointer', opacity: saving ? .6 : 1 }} disabled={saving} onClick={save}>{saving ? 'Saving…' : 'Save Report'}</button>
              </div>
            </div>
            ...
            {preview.categories.length > 0 && (
              <div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr>{['Category', 'Items Sold', 'Revenue'].map((h, i) => (
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            <div style={{ display: 'inline-block', width: 24, height: 24, border: `2px solid ${T.border}`, borderTopColor: T.sage, borderRadius: '50%', animation: 'spin .7s linear infinite' }} />
          </div>
        ) : reports.length === 0 ? (
          !preview && <p style={{ textAlign: 'center', color: '#9ca3af', fontSize: 13, padding: '20px 0' }}>No reports uploaded yet. Drop your first Square .eml above.</p>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>{['Month','Gross Sales','Tax','Fees','Net Payout','Top Category',''].map((h, i) => (
              ...
              {reports.map((r, i) => {
                ...
                return (
                  <tr key={r.id} style={{ background: i % 2 === 0 ? '#fff' : '#f9fafb', borderBottom: `1px solid ${T.border}` }}>
                    <td style={{ padding: '8px 10px', fontSize: 13, fontWeight: 500, color: T.sage }}>{fmtPeriod(r.period)}</td>
                    ...
                    <td style={{ padding: '8px 10px', fontSize: 12, textAlign: 'right', color: T.charcoal }}>{top ? `${top.name} (${fmt(top.amount)})` : '—'}</td>
                    <td style={{ padding: '8px 10px', textAlign: 'center' }}>
                      <button onClick={() => deleteReport(r.id)} style={{ background: 'none', border: 'none', color: '#9ca3af', cursor: 'pointer', fontSize: 13, padding: '2px 6px' }} title="Delete">✕</button>
                    </td>
                  </tr>
                )
              })}
