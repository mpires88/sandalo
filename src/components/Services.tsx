...
const BLANK: ServiceForm = {
  name: '', description: '', category_id: '', duration_minutes: '60',
  buffer_before_minutes: '0', buffer_after_minutes: '0',
  staff_count: '1', customer_count: '1',
  price: '', deposit_amount: '', resource_id: '',
  required_license_type: '', sort_order: '', is_active: true,
}
...
export default function Services() {
    ...
    try {
      const [sRes, rRes, cRes] = await Promise.all([
        supabase.from('services')
          .select('id, name, description, category, category_id, duration_minutes, buffer_before_minutes, buffer_after_minutes, staff_count, customer_count, price, deposit_amount, resource_id, required_license_type, sort_order, is_active')
  ...
  function openEdit(s: Service) {
    setForm({
      ...
      price: String(s.price), deposit_amount: s.deposit_amount != null ? String(s.deposit_amount) : '',
      resource_id: s.resource_id ?? '', required_license_type: s.required_license_type ?? '',
      sort_order: String(s.sort_order ?? ''), is_active: s.is_active,
    })
    setFormErr(''); setModal(true)
  }
  ...
  async function save() {
    ...
    if (!form.category_id)    { setFormErr('Category is required.'); return }
    ...
    try {
      ...
      const payload = {
        ...
        buffer_before_minutes: parseInt(form.buffer_before_minutes) || 0,
        buffer_after_minutes:  parseInt(form.buffer_after_minutes)  || 0,
        staff_count:    Math.max(1, parseInt(form.staff_count)    || 1),
        customer_count: Math.max(1, parseInt(form.customer_count) || 1),
    ...
    <div style={{ padding: '28px 32px', background: D.page, minHeight: '100vh' }}>
        ...
        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
          {[{ id: 'all', name: 'All', color: D.charcoal }, ...categories].map(cat => (
            <button key={cat.id} onClick={() => setCategoryFilter(cat.id)} style={{
              ...
              borderRadius: 5, padding: '4px 11px', fontSize: 12,
              fontWeight: categoryFilter === cat.id ? 600 : 400, cursor: 'pointer',
            }}>
              {cat.name}
            </button>
          ))}
      ...
      <div style={{ background: D.card, border: `1px solid ${D.border}`, borderRadius: 8, overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ padding: 40, textAlign: 'center', color: D.muted, fontSize: 13 }}>
            {categoryFilter !== 'all' ? `No services in ${categoryFilter}.` : 'No services yet — click "+ Add Service" to create one.'}
          </div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5 }}>
            <thead>
              <tr style={{ background: '#F0EBE0', borderBottom: `1px solid ${D.border}` }}>
                {['Name', 'Category', 'Duration', 'Buffer', 'Staff / Clients', 'Price', 'Deposit', 'Resource', 'Status', ''].map(h => (
            ...
            <tbody>
              {visible.map((s, i) => {
                ...
                return (
                  <tr key={s.id} style={{ borderBottom: i < visible.length - 1 ? `1px solid ${D.border}` : 'none', background: i % 2 === 0 ? D.card : '#F7F4EE', opacity: s.is_active ? 1 : 0.6 }}>
                    <td style={{ padding: '11px 14px', maxWidth: 280 }}>
                      <div style={{ fontWeight: 600, color: D.charcoal, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.name}</div>
                      {s.description && <div style={{ fontSize: 11, color: D.muted, marginTop: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.description}</div>}
                    </td>
                    <td style={{ padding: '11px 14px', whiteSpace: 'nowrap' }}>
                      {cat ? (
                        <span style={{ background: cat.color + '18', border: `1px solid ${cat.color}40`, borderRadius: 4, padding: '2px 7px', fontSize: 11, fontWeight: 500, color: cat.color, display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                    ...
                    <td style={{ padding: '11px 14px', whiteSpace: 'nowrap' }}>
                      {!hasBefore && !hasAfter ? (
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                          {hasBefore && <span style={{ fontSize: 11, color: '#1A6EAD' }}>↓ {s.buffer_before_minutes}m prep</span>}
                          {hasAfter  && <span style={{ fontSize: 11, color: '#B87400' }}>↑ {s.buffer_after_minutes}m cleanup</span>}
                        </div>
                      )}
                    </td>
                    <td style={{ padding: '11px 14px', whiteSpace: 'nowrap' }}>
                    ...
                    <td style={{ padding: '11px 14px', color: D.charcoal, whiteSpace: 'nowrap', fontWeight: 500 }}>{fmtMoney(s.price)}</td>
                    <td style={{ padding: '11px 14px', color: s.deposit_amount ? D.charcoal : D.muted, whiteSpace: 'nowrap' }}>{fmtMoney(s.deposit_amount)}</td>
                    <td style={{ padding: '11px 14px', whiteSpace: 'nowrap' }}>
                      {resource
                    ...
                    <td style={{ padding: '11px 14px', whiteSpace: 'nowrap', textAlign: 'right' }}>
                      <button onClick={() => openEdit(s)} style={actionBtn}>Edit</button>
                      <button onClick={() => toggleActive(s)} style={{ ...actionBtn, marginLeft: 6 }}>
                        {s.is_active ? 'Deactivate' : 'Activate'}
                      </button>
                      <button onClick={() => { setConfirmDelete(s); setDeleteErr('') }} style={{ ...dangerBtn, marginLeft: 6 }}>Delete</button>
                    </td>
                  </tr>
                )
              })}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}
          ...
          <div style={{ background: '#fff', borderRadius: 10, width: 560, maxWidth: '95vw', maxHeight: '90vh', display: 'flex', flexDirection: 'column', boxShadow: '0 8px 32px rgba(0,0,0,0.18)' }}>
            <div style={{ padding: '20px 24px 14px', borderBottom: `1px solid ${D.border}` }}>
              <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: D.charcoal }}>{form.id ? 'Edit Service' : 'New Service'}</h2>
            </div>
            <div style={{ padding: '18px 24px', overflowY: 'auto', flex: 1 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '13px 16px' }}>
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  ...
                  <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. 1 hr Relaxation Massage" style={inp} autoFocus />
                </div>
                <div style={{ gridColumn: '1 / -1' }}>
                  <label style={lbl}>Description</label>
                  <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Brief description of what the service includes…" rows={2} style={{ ...inp, resize: 'vertical', fontFamily: 'inherit' }} />
                </div>
                <div>
                ...
                <div>
                  ...
                  <datalist id="license-list">
                    {LICENSE_TYPES.map(l => <option key={l} value={l} />)}
                  </datalist>
                </div>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 4, marginBottom: 6, flexWrap: 'wrap' }}>
                    {DURATION_PRESETS.map(p => (
                      <button key={p} type="button" onClick={() => setForm(f => ({ ...f, duration_minutes: String(p) }))}
                        style={{ background: form.duration_minutes === String(p) ? D.charcoal : 'transparent', color: form.duration_minutes === String(p) ? '#fff' : D.muted, border: `1px solid ${form.duration_minutes === String(p) ? D.charcoal : D.border}`, borderRadius: 4, padding: '2px 8px', fontSize: 11, cursor: 'pointer' }}>
                        {p}
                      </button>
                    ))}
                  </div>
                  <input type="number" min="5" step="5" value={form.duration_minutes} onChange={e => setForm(f => ({ ...f, duration_minutes: e.target.value }))} style={inp} />
                </div>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: 4, marginBottom: 6, flexWrap: 'wrap' }}>
                      {BUFFER_PRESETS.map(p => (
                        <button key={p} type="button" onClick={() => setForm(f => ({ ...f, buffer_before_minutes: String(p) }))}
                          style={{ background: form.buffer_before_minutes === String(p) ? '#1A6EAD' : 'transparent', color: form.buffer_before_minutes === String(p) ? '#fff' : D.muted, border: `1px solid ${form.buffer_before_minutes === String(p) ? '#1A6EAD' : D.border}`, borderRadius: 4, padding: '2px 7px', fontSize: 11, cursor: 'pointer' }}>
                          {p}
                        </button>
                      ))}
                    </div>
                    <input type="number" min="0" step="5" value={form.buffer_before_minutes} onChange={e => setForm(f => ({ ...f, buffer_before_minutes: e.target.value }))} placeholder="0" style={inp} />
                    <div style={{ fontSize: 10.5, color: D.muted, marginTop: 3 }}>Setup / room prep before service starts</div>
                  </div>
                  <div>
                    <label style={lbl}>Buffer After (cleanup)</label>
                    <div style={{ display: 'flex', gap: 4, marginBottom: 6, flexWrap: 'wrap' }}>
                      {BUFFER_PRESETS.map(p => (
                        <button key={p} type="button" onClick={() => setForm(f => ({ ...f, buffer_after_minutes: String(p) }))}
                          style={{ background: form.buffer_after_minutes === String(p) ? '#B87400' : 'transparent', color: form.buffer_after_minutes === String(p) ? '#fff' : D.muted, border: `1px solid ${form.buffer_after_minutes === String(p) ? '#B87400' : D.border}`, borderRadius: 4, padding: '2px 7px', fontSize: 11, cursor: 'pointer' }}>
                          {p}
                        </button>
                      ))}
                    </div>
                    <input type="number" min="0" step="5" value={form.buffer_after_minutes} onChange={e => setForm(f => ({ ...f, buffer_after_minutes: e.target.value }))} placeholder="0" style={inp} />
                    <div style={{ fontSize: 10.5, color: D.muted, marginTop: 3 }}>Changeover / cleanup after service ends</div>
                  </div>
                </div>
                {(parseInt(form.buffer_before_minutes) > 0 || parseInt(form.buffer_after_minutes) > 0) && (
                  <div style={{ gridColumn: '1 / -1', background: '#F0EBE0', border: `1px solid ${D.border}`, borderRadius: 5, padding: '7px 12px', fontSize: 12, color: D.charcoal }}>
                    Total slot time: <strong>{fmtDur((parseInt(form.buffer_before_minutes) || 0) + (parseInt(form.duration_minutes) || 0) + (parseInt(form.buffer_after_minutes) || 0))}</strong>
                    <span style={{ color: D.muted, marginLeft: 8 }}>({form.buffer_before_minutes || 0}m prep + {form.duration_minutes || 0}m service + {form.buffer_after_minutes || 0}m cleanup)</span>
                  </div>
                )}
                ...
                <div>
                  ...
                  <input type="number" min="1" max="10" step="1" value={form.staff_count} onChange={e => setForm(f => ({ ...f, staff_count: e.target.value }))} style={inp} />
                  <div style={{ fontSize: 10.5, color: D.muted, marginTop: 3 }}>Number of providers needed (1 for standard, 2 for couples)</div>
                </div>
                <div>
                  <label style={lbl}>Clients Served</label>
                  <input type="number" min="1" max="10" step="1" value={form.customer_count} onChange={e => setForm(f => ({ ...f, customer_count: e.target.value }))} style={inp} />
                  <div style={{ fontSize: 10.5, color: D.muted, marginTop: 3 }}>Number of clients in one booking (1 for individual, 2 for couples)</div>
                </div>
                ...
                <div>
                  ...
                  <input type="number" min="0" step="0.01" value={form.deposit_amount} onChange={e => setForm(f => ({ ...f, deposit_amount: e.target.value }))} placeholder="Optional" style={inp} />
                </div>
                ...
                <div>
                  ...
                  <select value={form.resource_id} onChange={e => setForm(f => ({ ...f, resource_id: e.target.value }))} style={inp}>
                ...
                <div>
                  ...
                  <input type="number" min="0" step="1" value={form.sort_order} onChange={e => setForm(f => ({ ...f, sort_order: e.target.value }))} placeholder="1" style={inp} />
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingTop: 20 }}>
                  <input type="checkbox" id="svc-active" checked={form.is_active} onChange={e => setForm(f => ({ ...f, is_active: e.target.checked }))} style={{ cursor: 'pointer' }} />
                  <label htmlFor="svc-active" style={{ fontSize: 12.5, color: D.charcoal, cursor: 'pointer' }}>Active</label>
                </div>
      ...
      {confirmDelete && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}
          ...
          <div style={{ background: '#fff', borderRadius: 10, width: 380, maxWidth: '95vw', boxShadow: '0 8px 32px rgba(0,0,0,0.18)', padding: '24px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
              ...
              <button onClick={deleteService} disabled={deleting} style={{ background: D.red, color: '#fff', border: 'none', borderRadius: 6, padding: '8px 18px', fontSize: 13, fontWeight: 600, cursor: deleting ? 'not-allowed' : 'pointer', opacity: deleting ? 0.7 : 1 }}>
