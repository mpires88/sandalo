...
export default function Resources() {
  ...
  function openEdit(r: Resource) {
    setForm({ id: r.id, name: r.name, quantity: String(r.quantity), sort_order: String(r.sort_order ?? ''), is_active: r.is_active })
    setFormErr(''); setModal(true)
  }
  ...
  async function save() {
    ...
    if (!qty || qty < 1) { setFormErr('Quantity must be at least 1.'); return }
    ...
    <div style={{ padding: '28px 32px', background: D.page, minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ margin: 0, fontSize: 12.5, color: D.muted }}>Treatment rooms, tables, and equipment shared across appointments</p>
        </div>
      ...
      {!loading && !error && (
        <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
          {[
            ...
            { label: 'Total Capacity',  value: resources.filter(r => r.is_active).reduce((s, r) => s + r.quantity, 0) + ' units' },
          ].map(c => (
      ...
      <div style={{ background: D.card, border: `1px solid ${D.border}`, borderRadius: 8, overflow: 'hidden' }}>
        {loading ? (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5 }}>
            ...
            <tbody>
              {visible.map((r, i) => (
                <tr key={r.id} style={{ borderBottom: i < visible.length - 1 ? `1px solid ${D.border}` : 'none', background: i % 2 === 0 ? D.card : '#F7F4EE', opacity: r.is_active ? 1 : 0.6 }}>
                  <td style={{ padding: '11px 14px', fontWeight: 600, color: D.charcoal }}>
                    {r.name}
                  </td>
                  <td style={{ padding: '11px 14px', color: D.charcoal }}>
                    <span style={{ background: D.sage + '18', color: D.sage, border: `1px solid ${D.sage}30`, borderRadius: 4, padding: '2px 9px', fontSize: 12, fontWeight: 600 }}>
                  ...
                  <td style={{ padding: '11px 14px', color: D.muted }}>
                    {usageCounts[r.id] != null ? `${usageCounts[r.id]} appointment${usageCounts[r.id] !== 1 ? 's' : ''}` : '—'}
                  </td>
                  ...
                  <td style={{ padding: '11px 14px', whiteSpace: 'nowrap', textAlign: 'right' }}>
                    ...
                    <button onClick={() => { setConfirmDelete(r); setDeleteErr('') }} style={{ ...dangerBtn, marginLeft: 6 }}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}
          ...
          <div style={{ background: '#fff', borderRadius: 10, width: 420, maxWidth: '95vw', boxShadow: '0 8px 32px rgba(0,0,0,0.18)' }}>
            <div style={{ padding: '20px 24px 14px', borderBottom: `1px solid ${D.border}` }}>
              <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: D.charcoal }}>{form.id ? 'Edit Resource' : 'New Resource'}</h2>
            </div>
            <div style={{ padding: '18px 24px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '13px 16px' }}>
                ...
                <div style={{ gridColumn: '1 / -1', display: 'flex', alignItems: 'center', gap: 8 }}>
                  <input
                    type="checkbox" id="res-active"
                    checked={form.is_active}
                    onChange={e => setForm(f => ({ ...f, is_active: e.target.checked }))}
                    style={{ cursor: 'pointer' }}
                  />
                  <label htmlFor="res-active" style={{ fontSize: 12.5, color: D.charcoal, cursor: 'pointer' }}>Active (available for booking)</label>
                </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}
          ...
          <div style={{ background: '#fff', borderRadius: 10, width: 380, maxWidth: '95vw', boxShadow: '0 8px 32px rgba(0,0,0,0.18)', padding: '24px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
              ...
              <button onClick={deleteResource} disabled={deleting} style={{ background: D.red, color: '#fff', border: 'none', borderRadius: 6, padding: '8px 18px', fontSize: 13, fontWeight: 600, cursor: deleting ? 'not-allowed' : 'pointer', opacity: deleting ? 0.7 : 1 }}>
