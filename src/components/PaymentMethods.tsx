...
export default function PaymentMethods() {
  ...
  function openAdd() {
    const nextOrder = methods.length > 0 ? Math.max(...methods.map(m => m.sort_order ?? 0)) + 1 : 1
  ...
  async function save() {
    ...
    if (!form.code.trim()) {
      setFormErr('Code is required.')
      return
    }
    if (!/^[a-z0-9_]+$/.test(form.code)) {
      setFormErr('Code must be lowercase letters, numbers, and underscores only.')
      return
    }
    setSaving(true)
    setFormErr('')
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ margin: 0, fontSize: 12.5, color: D.muted }}>
            Allowed payment options across appointments and check-ins
          </p>
        </div>
      ...
      <div style={{ background: D.card, border: `1px solid ${D.border}`, borderRadius: 8, overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ padding: 40, textAlign: 'center', color: D.muted, fontSize: 13 }}>
            No payment methods. Click "+ Add Method" to create one.
          </div>
        ) : (
            ...
            <div style={{ padding: '18px 24px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '13px 16px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingTop: 20 }}>
                  ...
                  />
                  <label htmlFor="pm-active" style={{ fontSize: 12.5, color: D.charcoal, cursor: 'pointer' }}>
                    Active
                  </label>
                </div>
        ...
        >
          ...
          >
            ...
            <p style={{ margin: '0 0 16px', fontSize: 13, color: D.charcoal }}>
              <strong>{confirmDelete.name}</strong> (<code style={{ fontSize: 12 }}>{confirmDelete.code}</code>) will be
              permanently removed.
            </p>
            {deleteErr && (
              <div
