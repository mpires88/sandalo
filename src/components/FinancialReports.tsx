...
import { mergeAccounts, type MergedAccount } from '@/lib/chartOfAccounts'
...
export default function FinancialReports() {
  ...
  const availableYears = useMemo(() => Array.from(new Set(allMonths.map(m => m.slice(0, 4)))).sort(), [allMonths])
  ...
  const displayPeriods = useMemo(
    () =>
      periodMode === 'monthly' && mode === 'pl' && basePeriods.length > 1 ? [...basePeriods, 'total'] : basePeriods,
  ...
  function accountTotal(name: string, section: string, key: string): number {
    ...
    return dispAmt(name, section, key) + kids.reduce((s, c) => s + accountTotal(c.name, section, key), 0) + faContrib
  }
  ...
  function sectionTotal(section: string, key: string): number {
    return (bySection[section] ?? []).filter(a => !a.parent).reduce((s, a) => s + accountTotal(a.name, section, key), 0)
  }
  ...
  function bsSummary(key: string) {
    ...
    const unassignedLiabs = finAccounts.filter(
      a => accountSection(a.account_type) === 'liability' && !a.parent_category,
    )
  ...
  const colW =
    periodMode === 'yearly' ? (N <= 3 ? 80 : N <= 8 ? 70 : 60) : N <= 3 ? 110 : N <= 8 ? 85 : N <= 14 ? 70 : 58
    ...
    return (
      <tr key={`sh-${label}`}>
        ...
        {displayPeriods.map(k => (
          <td key={k} style={{ background: D.sage }} />
        ))}
      </tr>
    )
  }
      ...
      <tr key={`sum-${label}`} style={{ background: bg }}>
        <td
          style={{
            ...
            fontWeight: 700,
            fontSize: 13,
            color: highlight ? '#fff' : D.charcoal,
          }}
        >
          {label}
        </td>
  ...
  function groupHeaderRow(label: string): React.ReactNode {
    return (
      <tr key={`grp-${label}`}>
        ...
        {displayPeriods.map(k => (
          <td key={k} style={{ background: BG.groupHeader, borderBottom: `1px solid ${D.border}` }} />
        ))}
      </tr>
    )
  }
      ...
      >
        {displayPeriods.length === 0 ? (
          ...
          <table style={{ borderCollapse: 'collapse', fontSize: 12.5 }}>
            ...
            <tbody>{mode === 'pl' ? plRows() : bsRows()}</tbody>
          </table>
        )}
      </div>
