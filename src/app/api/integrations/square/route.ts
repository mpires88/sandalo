...
export async function POST(request: Request) {
  ...
  if (!locRes.ok) {
    ...
    return NextResponse.json({ error: msg ?? `Square rejected the token (HTTP ${locRes.status}). Check the token and environment.` }, { status: 400 })
  }
  const locations = ((locRes.json as { locations?: Array<{ id: string; name: string; merchant_id: string; status: string }> })?.locations ?? [])
    .filter(l => l.status !== 'INACTIVE')
  if (locations.length === 0) return NextResponse.json({ error: 'Token is valid but the account has no active locations.' }, { status: 400 })
  ...
  const db = adminClient()
  const { data: row, error } = await db.from('square_connections').upsert({
    client_id: CLIENT_ID,
    environment,
    access_token: token,
...
export async function PATCH(request: Request) {
  ...
  const { data: updated } = await db.from('square_connections')
    .update({ location_id: chosen.id, location_name: chosen.name }).eq('client_id', CLIENT_ID).select('*').single()
